...
テニスコート専用のホワイトボード。コート上にキャラクター・配球・移動矢印・
...
- 拡大縮小: ピンチ、右下の ＋ − ボタン（マウスホイールでの拡大縮小はありません）
- コート自体はドラッグでは動きません（誤操作防止のため、パン機能はありません）
...
`Background → Court → Drawing → Movement → Shot → Character → Text`

キャラクターは配球ラインより上に描かれます。
